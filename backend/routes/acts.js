...
router.get('/', async (req, res) => {
    ...
    if (!foundActs || foundActs.length === 0) return res.status(400).send('No acts')
...
router.get('/:actId', async (req, res) => {
    ...
    if (!foundAct || foundAct.length === 0) return res.status(400).send({ 'success': false })
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (!_id || _id.length !== 24) res.status(404).send('The act with the given ID was not found.')
...
router.delete('/:actId', async (req, res) => {
    ...
    if (!foundAct) return res.status(404).send('The act with the given ID was not found.')
